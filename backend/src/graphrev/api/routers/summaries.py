...
async def demand_summary(
    ...
    result = await summary_service.demand_summary(
        session, queue, function_id=function_id, priority=request.priority
    )
...
async def release_summary(function_id: int, queue: SummaryQueueDep) -> None:
    """Release demand / cancel if unstarted (endpoint 18, C8). Advisory only
    — an in-flight generation is never interrupted."""
    summary_service.release_summary_demand(queue, function_id=function_id)
...
async def regenerate_summary(
    function_id: int, session: SessionDep, queue: SummaryQueueDep
) -> SummaryDemandResponseDto:
    """Force regeneration, bypassing the cache (endpoint 19, C7)."""
    return await summary_service.regenerate_summary(session, queue, function_id=function_id)
